...
const Command: ICommand = {
  ...
  callback: async ({ message, client, args }) => {
    ...
    if (parseInt(args[0]) > 100) return message.reply("You can't delete more than 100 messages!").then(msg => { msg.delete({ timeout: 3000 }) }).catch(console.error);

    if (parseInt(args[0]) < 1) return message.reply("You must delete at least 1 message!").then(msg => { msg.delete({ timeout: 3000 }) }).catch(console.error);
