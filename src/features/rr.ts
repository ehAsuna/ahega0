...
const cache: { [guildId: string]: Reaction } = {} // { guildId: [message, { Emoji: RoleID }] }

const fetchCache = (guildId: string) => cache[guildId] || []

async function addToCache(guildId: string, message: Discord.Message, emoji?: string, roleId?: string) {
...
async function handleReaction(reaction: Discord.MessageReaction, user: Discord.PartialUser | Discord.User, adding: boolean) {
...
export default async (client: Discord.Client) => {
  ...
  for (const result of results) {
    ...
    const channel = await guild.channels.cache.get(channelId) as Discord.TextChannel;
