...
export default (client: Discord.Client) => {
	client.on("debug", (info) => {
		const dbCh = (client.channels.cache.find(channel => (channel as TextChannel).name === '╙┈🤖-suggest-ideas') as TextChannel).send(info); //???
	});
	...
	client.on("rateLimit", (rateLimitInfo) => {
		...
		(client.channels.cache.find(channel => (channel as TextChannel).name === '╙┈🤖-rate-limits') as TextChannel).send(rlEm);
	});
	...
	client.on('guildCreate', (guild) => {
		...
		let channel = guild.channels.cache.get(guild.systemChannelID || channelID!) as TextChannel;
		...
		(client.channels.cache.get('844032081353113620') as TextChannel).send(newServ);
	});
	...
	client.on('guildDelete', (guild) => {
		...
		(client.channels.cache.get('844032081353113620') as TextChannel).send(leaveServ)
	});
}